...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/03/18 21:43:27 by itsuman           #+#    #+#             */
/*   Updated: 2019/03/18 21:43:32 by itsuman          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
unsigned char	*step2_sha256(unsigned char *msg, t_container *var)
{
	...
	while (i < 4)
	{
		msg[g_size - 4 + i] = (unsigned char)(length_bit >> i * 8);
		i++;
	}
	return (msg);
}
...
unsigned int	*step3_sha256(unsigned char *msg)
{
	...
	x = (unsigned int *)(msg);
	free(msg);
	x[(g_size / 4) - 1] = rev_bit(x[(g_size / 4) - 1]);
	return (x);
}

void			initialize_t(t_variables *var)
{
...
void			initialize_h(t_variables *var)
{
	var->h0 = 0x6A09E667;
	var->h1 = 0xBB67AE85;
	var->h2 = 0x3C6EF372;
...
void			calculate_sha256(t_container **container)
{
	...
	while (cnt)
	{
				...
				(cnt->flag == 'f' && cnt->is_valid == 1))
		{
			...
			main_loop_sha256(x, &var);
			print_output_sha256(&var, cnt);
		}
		cnt = cnt->next;
	}
}
